...
def vgg_page():
    vgg_model = tf.keras.models.load_model("../modelling/vgg_model.h5")
    
    with open("../modelling/label_vgg.json", "r") as f:
        label_dict = json.load(f)

    # index → label
    class_names = {v: k for k, v in label_dict.items()}

    st.subheader("Klasifikasi Citra BISINDO (VGG16)")
